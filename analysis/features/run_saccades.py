...
def run_saccades(cfg: dict) -> None:
        ...
        for trial_id, streams in data.items():
            ...
            for sac_id, sac in enumerate(end_saccades, start=1):
                start_t, end_t, duration, xs, ys, xe, ye = sac
                    ...
                    #continue
                amplitude = float(((xe - xs) ** 2 + (ye - ys) ** 2) ** 0.5)  # amplitude is usually the distance the eye moves during one saccade.
                rows.append({
                    "saccade_id":  sac_id,
            ...
            if rows:
                ...
                sac_df.to_csv(out_dir / "saccades.csv", index=False)    #sac_df contains all saccades for the current trial, with one row per saccade.
    ...
    if best_summaries:
        ...
        best_out = processed / "saccades_best_run_summary.csv"
